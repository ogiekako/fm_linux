...
print_record(FILE *Fp)
{
	...
	fputs("解析時間：", Fp);
	if (hours > 0) {
		fprintf(Fp, "%d時間 %2d分 %2d秒  ", hours, minutes, seconds);
	} else if (minutes > 0) {
		fprintf(Fp, "%d分 %2d秒  ", minutes, seconds);
	} else {
		fprintf(Fp, "%d秒  ", seconds);
	}
	fprintf(Fp, "解析局面数：%s  検出解数：%ld",
...
print_rule(FILE *Fp)
{
	...
	reg_rule(outbuf);
	fputs(outbuf, Fp);
	fprintf(Fp, " %s手", make_num_str(0, limit_depth, work));
	if (max_limit_depth > 0) {
		...
		fprintf(Fp, "-%s）", make_num_str(0, max_limit_depth, work));
	}
	fprintf(Fp, "\n");
	...
	for (pc = PC_F1; pc <= PC_F7; pc++) {
		if (piece[pc].attr & B_PA_DEFINED) {
			...
			if (piece[pc].attr & B_PA_ROYAL) {
				sprintf(outbuf, "%s: %s王", graph[pc], piece[pc].name);
			} else {
			...
			if (piece[pc].attr & B_PA_PROMOTABLE) {
				...
				if (outbuf[x] != ' ' && NEQUALZ(outbuf + x - 1, "）")) {
					strcat(outbuf, " ");
				}
				strcat(outbuf, ">> ");
	...
	if (fileFp != NULL) {
		if (brief_output == 0) {
			...
			if (limit_depth > 0) {
				...
				if (verbose == ON) {
					fprintf(fileFp, "  生成手数 節＋葉：%s+%s=%s(%4.1f%%)\n",
							...
							(double)total_phase_count / (call_move_count + call_move_leaf_count) *
								100);
					fprintf(fileFp, "生成局面数 節＋葉：%s+%s=%s(%4.1f%%)\n",
...
// 詰手順木剪定
//   mmp:  処理対象詰手
//   mmq:  分岐詰手（NULLの場合は、mmpが初手の最初であることを示す）
static int
prune_mate_move_tree(mate_move_t *mmp, mate_move_t *mmq)
{
	...
	if (mmp->side != NULL) {
		...
		// 同レベル要素をすべて処理した後、または、同レベル要素がなかった場合
		if (mmq != mmp) {
			// 同レベル要素がなかった場合
			if (mmp->prev != NULL) {
